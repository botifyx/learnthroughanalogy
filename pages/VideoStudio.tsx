...
export default function VideoStudio() {
    ...
    if (!apiKeySelected) {
        return (
            <div className="text-center bg-slate-800 p-8 rounded-lg ring-1 ring-slate-700">
                <h2 className="text-2xl font-bold mb-4">API Key Required</h2>
                <p className="text-slate-400 mb-6">This feature requires an API key for video generation. Please select a key to continue.</p>
                <p className="text-sm text-slate-500 mb-6">For more information, see the <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noopener noreferrer" className="text-violet-600 hover:underline">billing documentation</a>.</p>
    ...
    return (
        <div className="bg-slate-800 p-6 md:p-8 rounded-lg shadow-xl ring-1 ring-slate-700">
            ...
            <p className="text-center text-slate-400 mb-6">Bring your images to life. Generate a video from an image and a prompt.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                    ...
                    <div>
                        ...
                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-600 border-dashed rounded-md">
                            <div className="space-y-1 text-center">
                                ...
                                <label htmlFor="file-upload" className="relative cursor-pointer bg-slate-700 rounded-md font-medium text-violet-400 hover:text-violet-500 px-2"><span>Upload a file</span><input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handleFileChange} accept="image/*"/></label>
                            </div>
                        </div>
                    </div>
                    
                    <div>
                        <label htmlFor="prompt" className="block text-sm font-medium text-slate-300 mb-2">Prompt (Optional)</label>
                        <textarea id="prompt" rows={3} value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="e.g., The robot starts waving..." className="w-full p-3 bg-slate-700 border border-slate-600 rounded-lg focus:ring-2 focus:ring-violet-500 focus:outline-none text-slate-200"></textarea>
                    </div>
                    
                    <div>
                        <label htmlFor="aspectRatio" className="block text-sm font-medium text-slate-300 mb-2">Aspect Ratio</label>
                        <select id="aspectRatio" value={aspectRatio} onChange={e => setAspectRatio(e.target.value as '16:9' | '9:16')} className="w-full p-3 bg-slate-700 border border-slate-600 rounded-lg focus:ring-2 focus:ring-violet-500 focus:outline-none text-slate-200">
                ...
                <div className="flex items-center justify-center bg-slate-700 rounded-lg min-h-[400px] p-4">
                    ...
                    {result ? <video src={result} controls autoPlay loop className="max-w-full max-h-full object-contain rounded-md"/> : !isLoading && <p className="text-slate-500">Your video will appear here</p>}
                    {isLoading && (
