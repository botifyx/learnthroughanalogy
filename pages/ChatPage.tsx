...
const UserAvatar = () => (
  <div className="w-10 h-10 rounded-full flex-shrink-0 bg-gradient-to-br from-slate-200 to-slate-300 ring-2 ring-slate-300 flex items-center justify-center">
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-cyan-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
  </div>
);
                ...
                <div className="mt-4 border-t border-slate-600 pt-3">
                    ...
                    {message.sources.map((source, index) => (
                        <li key={index} className="flex items-center gap-2">
                            ...
                            <a href={source.uri} target="_blank" rel="noopener noreferrer" className="text-violet-400 hover:underline truncate">
...
const ChatOptionToggle: React.FC<{label: string, checked: boolean, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void}> = ({ label, checked, onChange }) => (
    <label className="relative inline-flex items-center cursor-pointer">
        ...
        <div className="w-9 h-5 bg-slate-600 rounded-full peer peer-focus:ring-2 peer-focus:ring-violet-500 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-violet-600"></div>
        <span className="ml-2 text-sm font-medium text-slate-300">{label}</span>
    </label>
);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-10rem)] bg-slate-800 rounded-lg shadow-xl ring-1 ring-slate-700 overflow-hidden">
      ...
      <div className="p-4 bg-slate-900 border-t border-slate-700">
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 mb-3 text-sm text-slate-600">
